...
class st_gcn(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, kernel_size,
        ...
        # d_model = out_channels * V_num
        # self.tranformer = TransformerLayer(d_model=d_model,num_heads=num_heads)
        if not residual:
    ...
    def forward(self, x, A):
        ...
        x = self.tcn(x) + res
        # bs, c, t, v = x.shape
        # x = x.permute(0,2,1,3).contiguous().view(bs, t, c*v)
        # x = self.tranformer(x)
        # x = x.view(bs,t,c,v).permute(0,2,1,3).contiguous()
        # if self.residual is nn.Sequential:
        #     x = self.tcn(x) + res
        # else:
        #     x = x + res
